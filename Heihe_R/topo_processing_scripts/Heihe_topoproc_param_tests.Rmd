...
#channel 
channelR=raster("river_mask_edit7_capture_north_mount.tif")
...
#Set up masks to do 1m stream burn into DEM

##Mask out the channel cells outside the domain
channelmT = channelT * maskT

# add a 1m stream channel burn
demT=demT-channelmT
...
#```{r}
#setwd('/Users/amanda_triplett/Documents/Heihe_Basin_Project/parking_lot_test/diff_slope_files_RM_edit7')
#file_name = paste0("test15_stream_seg_sub_bas.png")
#png(filename=file_name, width = 800, height = 600, res = 100)
#par(mfrow=c(1,2))
...
```{r}
slopesCalc=SlopeCalStan(dem=RivSmooth$dem.adj, mask=lakemaskT, direction=travHS$direction, minslope=1e-4, maxslope=1, dx=1000, dy=1000, secondaryTH=-1) 
#Mask used is lakemaskT, all cells will be processed but terminal lakes

#Changes the slope of flat cells surrounded by steeper cells going into it (fast drainage into the cell, slow drainage out)
FlatFix=FixFlat(direction=travHS$direction, slopex=slopesCalc$slopex, slopey=slopesCalc$slopey, adj_th=1e-3, adj_ratio=-1, mask=lakemaskT)
```
```{r}
#plots the slopes coming out of lines 353 using RivSmooth$dem.adj for topoprocessing troubleshooting

slopex=FlatFix$slopex
slopey=FlatFix$slopey
...
slopesCalc2=RivSlope(direction=travHS$direction, slopex=FlatFix$slopex, slopey=FlatFix$slopey, minslope=1e-4, RiverMask=river.mask, Remove.Sec=TRUE)
```
...
#plot
setwd('/Users/amanda_triplett/Documents/Heihe_Basin_Project/parking_lot_test/diff_slope_files_RM_edit7')
file_name = paste0("test15_slopes.png")
png(filename=file_name, width = 800, height = 600, res = 100)
par(mfrow=c(1,2))
image.plot(sxplot)
image.plot(syplot)
...
#set where your want slopes to save
setwd('/Users/amanda_triplett/Documents/Heihe_Basin_Project/parking_lot_test/diff_slope_files_RM_edit7')

write.table( t(c(nx,ny,1)), "test15_SlopeX.sa", append=F, row.names=F, col.names=F)
#write.table(slopexPF, fout, append=T, row.names=F, col.names=F)
write.table(slopexPF, "test15_SlopeX.sa", append=T, row.names=F, col.names=F)

write.table( t(c(nx,ny,1)), "test15_SlopeY.sa", append=F, row.names=F, col.names=F)
#write.table(slopeyPF, fout, append=T, row.names=F, col.names=F)
write.table(slopeyPF, "test15_SlopeY.sa", append=T, row.names=F, col.names=F)

write.table( t(c(nx,ny,1)), "test15_DEM.Processed.sa", append=F, row.names=F, col.names=F)
#write.table(DEMPF, fout, append=T, row.names=F, col.names=F)
write.table(DEMPF, "test15_DEM.Processed.sa", append=T, row.names=F, col.names=F)

write.table( t(c(nx,ny,1)), "test15_Flow.Direction.sa", append=F, row.names=F, col.names=F)
#write.table(DIRPF, fout, append=T, row.names=F, col.names=F)
write.table(DIRPF, "test15_Flow.Direction.sa", append=T, row.names=F, col.names=F)
