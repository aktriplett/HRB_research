...
pf_ssn_map = array(0,dim = c(nx,ny,1)) #creates an empty one layer array to hold
#combo ss / n indicator
#Check the ss / n values of each cell and assign indicator 1-6
for (j in 1:nx){ #loops through all rows
        for (i in 1:ny){ #loops through all cols
                ssval = pf_ss[i,j,1] #gets ss value at current cell 
                ssval = round(ssval, digits = 5)
                #print(ssval)
                nval = pf_porosity[i,j,1] #gets porosity value at current cell
                #print(nval)
                if (ssval == allss_vals[1]){ #checks if ss is value 1 (1e-5)
                        if(nval == allporosity_vals[1]){ #checks if porosity is value 1 (.1)
                                #print('1 assigned')
                                pf_ssn_map[i,j,1] = 1 #gives indicator of 1 for (1,1) pair
                        }
                        ...
                        pf_ssn_map[i,j,1] = 0 #assigns 0, because if porosity is 0 we are outside domain
                }
        }
}
...
pf_ssn_map_5L = array(0,dim = c(nx,ny,5))
...
pf_ind_map=array(0,dim = c(nx,ny,nz)) #empty indicator map with correct x,y,z dim

for (j in 1:ny){ #loops through all rows
        for (i in 1:nx){ #loops through all cols
                depth_temp = hf_depth_map[i,j,] #"cookie cutter" of i/j cell with a bottom depth value for each layer
                ind_temp = pf_ssn_map_5L[i,j,] #"cookie cutter" of cell attributes (Ex. K) of each layer
