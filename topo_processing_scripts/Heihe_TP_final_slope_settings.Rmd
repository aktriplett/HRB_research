...
knitr::opts_knit$set(root.dir = "/Users/amanda_triplett/Documents/Heihe_Basin_Project/topographic_processing/input_data") #CHANGE to your working directory 
rm(list=ls())
...
#file_name = paste0("test14_stream_seg_sub_bas.png")
...
FlatFix=FixFlat(direction=travHS$direction, slopex=slopesCalc$slopex, slopey=slopesCalc$slopey, adj_th=1e-3, adj_ratio=100, mask=lakemaskT)
```
```{r}
...
#plot
setwd('/Users/amanda_triplett/Documents/Heihe_Basin_Project/parking_lot_test/diff_slope_files_RM_edit7')
file_name = paste0("test14_slopes.png")
png(filename=file_name, width = 800, height = 600, res = 100)
par(mfrow=c(1,2))
image.plot(sxplot)
image.plot(syplot)
...
write.table( t(c(nx,ny,1)), "test14_SlopeX.sa", append=F, row.names=F, col.names=F)
#write.table(slopexPF, fout, append=T, row.names=F, col.names=F)
write.table(slopexPF, "test14_SlopeX.sa", append=T, row.names=F, col.names=F)

write.table( t(c(nx,ny,1)), "test14_SlopeY.sa", append=F, row.names=F, col.names=F)
#write.table(slopeyPF, fout, append=T, row.names=F, col.names=F)
write.table(slopeyPF, "test14_SlopeY.sa", append=T, row.names=F, col.names=F)

write.table( t(c(nx,ny,1)), "test14_DEM.Processed.sa", append=F, row.names=F, col.names=F)
#write.table(DEMPF, fout, append=T, row.names=F, col.names=F)
write.table(DEMPF, "test14_DEM.Processed.sa", append=T, row.names=F, col.names=F)

write.table( t(c(nx,ny,1)), "test14_Flow.Direction.sa", append=F, row.names=F, col.names=F)
#write.table(DIRPF, fout, append=T, row.names=F, col.names=F)
write.table(DIRPF, "test14_Flow.Direction.sa", append=T, row.names=F, col.names=F)
